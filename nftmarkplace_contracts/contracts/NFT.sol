...
contract NFT is ERC721Enumerable, Ownable {
    ...
    // NFT Name
    string public constant TOKEN_NAME = "PlanetNeo NFT";
    // NFT Symbol
    string public constant TOKEN_SYMBOL = "PLA";
    ...
    constructor(string memory BASEURI) ERC721(TOKEN_NAME, TOKEN_SYMBOL) {
        ...
        presaleLimit = 5;
        publicSaleLimit = 20;
        presalePrice = 60_000_000_000_000_000; // 0.06
        publicSalePrice = 160_000_000_000_000_000; // 0.16
    }
    ...
    function setPresaleLimit(uint256 counter) external onlyOwner {
        require(counter > 0, "PLA: Presale amount limit can't be zero.");
    ...
    function setPublicSaleLimit(uint256 counter) external onlyOwner {
        require(counter > 0, "PLA: Public sale amount limit can't be zero.");
    ...
    function setPresalePrice(uint256 price) external onlyOwner {
        require(price > 0, "PLA: Presale amount can't be zero.");
    ...
    function setPublicSalePrice(uint256 price) external onlyOwner {
        require(price > 0, "PLA: Public sale amount can't be zero.");
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < clients.length; i++) {
            require(
                clients[i] != address(0),
                "PLA: Zero address can't be added to the whitelist."
            );
        }
        for (uint256 i = 0; i < clients.length; i++) {
    ...
    function presaleMint(uint256 amount) internal {
        require(
            ...
            "PLA: You are not added to the whitelist."
        );
        require(
            presalePrice * amount == msg.value,
            "PLA: Your presale payment amount does not match required presale minting amount."
        );
        require(
            (amount + _whitelistSold[msg.sender]) <= presaleLimit,
            "PLA: Your presale payment amount exceeds our presale minting amount limit."
        );
    ...
    function publicSaleMint(uint256 amount) internal {
        require(publicSaleStatus == true, "PLA: Public sale is not live.");
        require(
            publicSalePrice * amount == msg.value,
            "PLA: Your public sale payment amount does not match required presale minting amount."
        );
        require(
    ...
    function clientMint(uint256 amount) external payable {
        require(amount > 0, "PLA: Mint amount can't be zero");
        require(
            (getTotalMintNumber() + amount) <= maxTotalSupply,
            "PLA: You can't mint that amount of tokens. Exceeds max supply."
        );
    ...
    function adminAirdrop(address client, uint256[] calldata tokenIdArray)
        ...
        onlyOwner
    {   
        require(client != address(0), "PLA: You can't airdrop to the zero address");
        for (uint256 i = 0; i < tokenIdArray.length; i++) {
            require(
                tokenIdArray[i] < maxTotalSupply,
                "PLA: You can't airdrop that amount of tokens. Exceeds max supply."
            );
            require(
                _nftSoldList[tokenIdArray[i]] == false,
                "PLA: You can't airdrop token already minted."
            );
        }
        for (uint256 i = 0; i < tokenIdArray.length; i++) {
    ...
    function withdrawAdmin(address payable to, uint256 value)
        ...
        onlyOwner
    {
        require(to != address(0), "PLA: Can't withdraw to the zero address.");
        require(
            value <= address(this).balance,
            "PLA: Withdraw amount exceed the balance of this contract."
        );
        to.transfer(value);
