...
contract PresaleWCT is Ownable {
    ...
    function _unlockAmount(address buyer) internal view returns (uint256){
        ...
        for(uint256 i = unlockPeriod.length - 1; i >= 0; i--) {
            if(duration >= unlockPeriod[i].minDays.mul(120)){
            // if(duration >= unlockPeriod[i].minDays.mul(60)){
     ...
     */
    function userBalances(address buyer) external view returns (uint256, uint256, uint256, uint256){
        uint256 pendingUnlock = _unlockAmount(buyer) - lockedBalances[buyer].unlockedAmount;
        return (lockedBalances[buyer].totalAirdrop, lockedBalances[buyer].unlockedAmount, pendingUnlock, lockedBalances[buyer].time);
    }
    ...
    function _addBalance(address receiver, uint256 amountWCT) internal {
        ...
        lockedBalances[receiver].time = block.timestamp;
    }
