...
export default function Landing() {
  ...
  const { connect, account, chainId, signer, provider, library } =
  ...
  const getMintabeInfo = useCallback(() => {
    ...
    ChainList.map((item) => {
      ...
      const getMintable = async () => {
        let _mintedCount = await pozzlenautsONFT.countMinted(account);
        _mintCount += Number(_mintedCount);
        setMintedCount(_mintCount);

        let _isFLClaim = await pozzlenautsONFT.isFLClaimed(account);
        let _isGLClaim = await pozzlenautsONFT.isGLClaimed(account);
  ...
  const getAllowance = useCallback(async () => {
    if (!account) return;
    if (chainOn === -1) return;
    if (chainOn !== chainId) return;
    if (account !== signer._address) return;
    if (MockUSDC[chainId] === nullAddress || MockUSDC[chainId] === undefined)
      return;

    setPending(true);
    let mockUSDC = new ethers.Contract(
      MockUSDC[chainId],
      JSON.stringify(USDC_ABI[chainId]),
      signer
    );
  ...
  const performVerify = async () => {
    ...
    setVerifyModalVisible(false);
    let rawMessage = `Please sign to let us verify that you are the owner of this address: ${account} \n\n [${new Date(
      Date.now()
    ...
    try {
      if (library.provider) {
        signedMessage = await library.provider.send("personal_sign", [
  ...
  const peformLatestNFTAddress = (_chainId) => {
    ...
    switch (_chainId) {
      ...
      case ChainID.mumbai:
        link = `https://${
          isTest ? "testnets." : ""
        }opensea.io/assets?search[resultModel]=ASSETS&search[query]=${account}&search[chains][0]=${
          isTest
        ...
        break;
      default:
        link = `https://tofunft.com/user/${account}/items/in-wallet?contract=${_chainId}_${PozzlenautsONFT[_chainId]}&network=${_chainId}`;
        break;
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      let balance = await mockUSDC.balanceOf(account);
    ...
    ChainList.map((item) => {
      ...
      const getOwner = async () => {
        try {
          ...
          if (_ownerOf === account) {
            if (item.id === chainId || chainFrom === item.id)
              setChainFrom(item.id);
            else {
              await switchNetwork(item.id, false);
            }
            setDetected(true);
            setPending(false);
            setPendingModalVisible(false);
  ...
  const handleTransfer = async () => {
    ...
    setPendingText(
      `Sending Pozzlenaut #${tokenId} from ${
        ChainList.find((item) => item.id === chainOn && item).chain
    ...
    let pozzlenautsONFT = new ethers.Contract(
      PozzlenautsONFT[chainId],
      JSON.stringify(
        chainId === ChainID.mumbai || chainId === ChainID.polygon
          ? POZZPoz_ABI
          : POZZ_ABI
      ),
      signer
  ...
  useEffect(() => {
    ...
    switchNetwork(chainFrom, false);
    setDetected(true);
    return () => {};
  }, [chainFrom, switchNetwork]);
  ...
  useEffect(() => {
    ...
    switchNetwork(ChainList[chainNewOn].id, true);
    setChainNewOn(-1);
    setDisabled(false);
    return () => {};
  }, [account, chainNewOn, switchNetwork]);
  ...
  useEffect(() => {
    if (isDetected) {
      try {
        toast.success(
          `Pozzlenaut #${tokenId} is detected in ${
            ChainList.find((item) => item.id === chainFrom && item).chain
          }`,
          {
            type: toast.TYPE.SUCCESS,
            icon: (
          ...
          <>
            ...
            {!account ? (
              ...
              <Button
                className="btns btn-control"
                variant="purple"
                disabled={isPending || isDisabled || flClaimed}
              ...
              <Button
                className="btns btn-control"
                variant="purple"
                disabled={isPending || isDisabled || isLimited}
              ...
              <Button
                className="btns btn-control"
                variant="purple"
                disabled={isPending || isDisabled || glClaimed}
