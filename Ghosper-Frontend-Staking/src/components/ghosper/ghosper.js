  ...
  */
  ...
  const get_balanceUrls = async () => {    
    ...
    await nft_balanceURIs.map(async (value, index) => {
      ...
      let assetURI = "";
      await fetch(value, requestOptions)
      .then(response => response.text())
      .then(result => assetURI = result)
      .catch(error => console.log('error', error));

      // let assetURI = await Axios.get(value, {
      //   mode:"no-cors",
      //   headers: {
      ...
      //   return "";
      // });
      assetURIs.push({id : nft_balanceIds[index], uri: ImageUrlFromIPFS(JSON.parse(assetURI).image)});
    });

    // console.log('assets------', assetURIs);
  ...
  const get_stakeUrls = async () => {    
    ...
    nft_stakeUris.map(async (value, index) => {
      ...
      assetURIs.push({id : balanceofstakes[index], uri: ImageUrlFromIPFS(JSON.parse(assetURI).image)});
    });

    // console.log('assetstake------', assetURIs);
